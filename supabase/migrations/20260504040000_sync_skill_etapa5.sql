-- Sync ai_agent_config.system_prompt with updated SKILL_CORE (Etapa 5: benchmark + proatividade)
...
INSERT INTO public.ai_agent_config_history (config_snapshot, changed_at, change_reason)
SELECT to_jsonb(c.*), now(), 'Etapa 5: benchmark RPC tool, proatividade rules, benchmark format'
FROM public.ai_agent_config c WHERE c.singleton = true;
