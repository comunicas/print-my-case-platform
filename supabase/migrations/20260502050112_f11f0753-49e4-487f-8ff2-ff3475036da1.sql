...
-- 2. ai_get_stock_overview: drop ambas as sobrecargas e recria
...
CREATE FUNCTION public.ai_get_stock_overview(
  _pdv_ids uuid[] DEFAULT NULL::uuid[],
  _limit integer DEFAULT 100,
  _product_name text DEFAULT NULL::text
)
RETURNS TABLE(
  product_name text,
  pdv_id uuid,
...
-- 3. ai_analyze_restock_targets: filtro case/space-insensitive por nome de produto
...
BEGIN
  ...
  WITH stock_agg AS (
    ...
    WHERE sr.is_active = true
      ...
      AND user_can_access_pdv(auth.uid(), sr.pdv_id)
      AND LOWER(TRIM(sr.product_name)) = ANY(SELECT LOWER(TRIM(n)) FROM UNNEST(_product_names) n)
    GROUP BY sr.product_name, sr.pdv_id, p.name
  ),
  sales_agg AS (
    ...
    WHERE s.status = 'Concluído'
      ...
      AND LOWER(TRIM(s.product_name)) = ANY(SELECT LOWER(TRIM(n)) FROM UNNEST(_product_names) n)
  ...
  pending AS (
    ...
    WHERE ps.organization_id = _org_id
      ...
      AND LOWER(TRIM(ps.product_name)) = ANY(SELECT LOWER(TRIM(n)) FROM UNNEST(_product_names) n)
